...
int main() {
    ...
    std::cout << std::setw(10) << "Note" << std::setw(5) << "nu" << std::setw(5) << "k"
              << std::setw(15) << "Frequency (Hz)" << std::setw(20) << "Wavelength (cm)" << std::endl;

    for (int nu = 0; nu <= k_nOctaves; ++nu) {
        for (int k = 0; k < k_nHalfTones; ++k) {
            // Calculate frequency
            double frequency = 16.35 * pow(dTwelfthRootOfTwo, nu + k / 12.0);
            ...
            // Display information
            std::cout << std::setw(10) << strNote << std::setw(5) << nu << std::setw(5) << k
                      << std::setw(15) << frequency << std::setw(20) << wavelength * 100 << std::endl;
        }
    }
